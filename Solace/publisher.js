...
var Publisher = function (topicName) {
  ...
  requestor.connect = function () {
    ...
    requestor.session.on(
      solace.SessionEventCode.UP_NOTICE,
      function (sessionEvent) {
        requestor.log(
          "=== Successfully connected and ready to send requests. ==="
        );
        requestor.request();
      }
    );
  ...
  requestor.request = function (question) {
    if (requestor.session !== null) {
      ...
      request.setDestination(
        solace.SolclientFactory.createTopicDestination(requestor.topicName)
      );
      ...
      requestor.log(
        "Cannot send request because not connected to Solace PubSub+ Event Broker."
      );
    }
  };
